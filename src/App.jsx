...
// CRM VIEW
// ═══════════════════════════════════════════════════════════
function CRMView({ leads, setLeads, setView }) {
  const t = useT();
  ...
  const addNote = () => { if (!noteText.trim() || !selId) return; setLeads(prev => prev.map(l => l.id === selId ? { ...l, notes: [...(l.notes || []), { text: noteText.trim(), time: new Date().toISOString() }] } : l)); setNoteText(""); };
  const moveStage = (id, s) => setLeads(prev => prev.map(l => l.id === id ? { ...l, stage: s } : l));
...
export default function App() {
  ...
  const [leads, setLeads] = useState(SAMPLE_LEADS.map(l => ({ ...l, notes: l.notes || [{ text: "Lead created via " + l.source, time: new Date(l.created + "T09:00:00").toISOString() }] })));
  ...
  return (
    <TC.Provider value={t}>
      <div style={{ minHeight: "100vh", background: t.bg, color: t.text, fontFamily: "'Source Sans 3', 'Segoe UI', -apple-system, sans-serif", paddingBottom: isCRM ? 0 : 56 }}>
        ...
        {view === "rebates" && <RebatesView onCaptureLead={l => setLeads(prev => [{ ...l, notes: [{ text: "Lead created via Rebate Calculator", time: new Date().toISOString() }] }, ...prev])} />}
        {view === "finance" && <FinanceView onCaptureLead={l => setLeads(prev => [l, ...prev])} setView={setView} />}
        {isCRM && <CRMView leads={leads} setLeads={setLeads} setView={setView} />}
      </div>
    </TC.Provider>
  );
}
